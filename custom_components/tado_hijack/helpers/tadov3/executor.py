...
class TadoV3Executor(TadoExecutorBase):
    ...
    async def _execute_zone_actions(self, merged: dict[str, Any]) -> None:
        ...
        # Overlays (heating, AC, hot water) share POST /overlay.
        # Bulk DELETE resume must not include HOT_WATER (v2 API).
        heating_resumes, overlays, hw_resumes = self._group_zone_actions(zones)
        ...
        if overlays:
            if real_overlays := self._filter_real_overlays(overlays):
        ...
        if hw_resumes:
            await self._execute_hw_resumes(hw_resumes, rollback_zones)

    def _group_zone_actions(
        self, zones: dict[int, dict[str, Any] | None]
    ) -> tuple[list[int], list[dict[str, Any]], dict[int, dict[str, Any] | None]]:
        """Split resumes vs overlays. HW overlay goes in the same POST as rooms."""
        resumes: list[int] = []
        overlays: list[dict[str, Any]] = []
        hw_resumes: dict[int, dict[str, Any] | None] = {}
        for zid, data in zones.items():
            z = self.coordinator.zones_meta.get(zid)
            if z and z.type == "HOT_WATER" and data is None:
                hw_resumes[zid] = data
            elif data is None:
                resumes.append(zid)
            else:
