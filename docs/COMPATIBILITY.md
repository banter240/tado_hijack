...
- **Active timetable (ONE_DAY / THREE_DAY / SEVEN_DAY):** Not part of Hops. GET/PUT `homes/{home_id}/zones/{zone_id}/schedule/activeTimetable` on the classic v2 API (per zone only; no bulk endpoint in the v2 OpenAPI). Tado X uses the same URI with room ids (experimental, like Energy IQ). Synthetic X hot water (zone 9001) is skipped. Writes and refreshes share the command debounce/batch window; a full manual poll also GETs every compatible zone.
