...
- **Offset auto-calibrate:** Home select `offset_cal_interval`. Only zones with a linked `zone_temp_source`. Offset = thermostat - TRV raw. Clock slots from local midnight in 3h steps, or once when quota reset is detected. Auto Quota subtracts those PUTs from the daily polling budget (`offset_cal_total` in reserved cost). Per device PUT, not bulk.
